...
def purifiedRefinedPOVM
    ...
    (a₀ : ι) : POVM ι e where
  operator := purifiedRefinedEffect F hF Γ operator a₀
  positive a := purifiedRefinedEffect_posSemidef F hF Γ hΓ
    operator hpositive a₀ a
  complete := purifiedRefinedEffect_complete F hF Γ operator hsum a₀
...
def coherentBinaryJointOutcome
  ...
  toLp 2
    (((P.operator a ⊗ₖ Q.operator b)).mulVec (ofLp z))
...
def binaryBornProbability
    ...
    (a b : Bool) : ℝ :=
  (Matrix.trace
    (ρ.matrix * (P.operator a ⊗ₖ Q.operator b))).re
...
def transposePOVM
  ...
  operator b := (P.operator b).transpose
